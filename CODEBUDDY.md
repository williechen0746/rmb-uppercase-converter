...
**两个参考板块用 `<details class="card">` 实现，默认收起、点击展开，且位于 `</footer>` 之后（页面最下方）**——用户要求如此，勿把它们移回卡片栈中间。样式见 `details.card` 系列规则。

> **不要给 `<summary>` 设 `display`。** Safari 下把 `<summary>` 改为 `flex` 等非 `list-item` 值会**失去点击展开能力**。flex 布局放在内层 `<span>` 上，`summary` 保持默认值。已在 CSS 中注释说明。
...
1. **折叠内容拿不到 `innerText`。** 两个板块现在是 `<details>` 默认收起，收起状态下其内容**不在 `innerText` 里**（`textContent` 仍在）。校验 FAQ 一致性前必须先 `d.open = true` 再取 `innerText`，否则会误判为"内容缺失"。
2. **不要用「空白归一为单空格 + `indexOf`」的方式比对 FAQ 文字**，会产生假阳性——`<b>`、`<code>` 等内联标签渲染不产生空格，而剥离标签时会被替换成空格。正确做法是**去掉全部空白字符后逐字比较**。
3. **点击视口外的元素会静默失败。** 页面最下方的 `<summary>` 初始位于视口之外，直接点击会返回成功却毫无效果。**必须先 `scrollintoview` 再点击**，否则会误判为"点击展开失效"。

另外，折叠后的正文**仍在 HTML 源码中**，爬虫与 `curl` 均可读到，因此不影响收录；但随着展开/收起策略变化，页面内可见文字量会变，评估"内容单薄"时应以展开后的量为准。
